...
//	BigDecimal trad_money
	private double trad_money = 0.0;//输入的交易金额
	private double costs = 0.0;//手续费
	BigDecimal cost;
...
//		panel.add(label_warn);
	...
	////////输入目标卡号   start     /////////////////////////////////////
		...
		panel.add(trad_to);
		trad_to.addKeyListener(new KeyAdapter() {
	...
	///*该行代码用于测试玩*/			if(!cardInfo.getCardInfo().getBank_code().trim().equals("")){
										...
										costs = trad_money*0.01;
										cost = new BigDecimal(costs).setScale(2, BigDecimal.ROUND_HALF_UP);
										System.out.println(cost);
										
										total_money = trad_money + costs;
										if(total_money <= current_money+curremt_credit_line){
											...
											if(total_money <= current_money){
												...
												BigDecimal to = new BigDecimal(total_money-current_money).setScale(2, BigDecimal.ROUND_HALF_UP);
												System.out.println(to);
												label_warn.setVisible(true);label_warn.setText("注意：手续费 ："+cost+"元，卡透支："+to+"确认转账？");
											}
										}else{
										...
										costs = trad_money*0.01 +trad_money*0.001;
										cost=BigDecimal.valueOf(costs);
										total_money = trad_money + costs;
										if(total_money <= current_money+curremt_credit_line){
											//换界面，
											label_tip.setText("请确认转账信息：");
											...
											if(total_money <= current_money){
												...
												BigDecimal to2 = BigDecimal.valueOf(total_money-current_money);
												label_warn.setVisible(true);label_warn.setText("注意：跨行和转账手续费 ："+cost+"元，卡需透支："+to2+"是否确认转账？");
											}
										}else{
	...
	/////    再次确认界面按钮        start      ///////////////////////////////////////
		...
		button_4 = new RButton("取消");
		button_4.setFont(new Font("黑体", Font.BOLD, 18));
		button_4.setBounds(0, 387, 133, 41);
